    ...
    <div>
        <el-row class="oneBar" v-for="(item, index) in categoryList" :key="index">
            <el-row justify="space-between" type="flex" class="oneBar-title">
                <el-col :span="3" class="title-name"><span>{{ item.name }}</span></el-col>
                <el-col :span="6" class="title-icon">
                    <el-row justify="end" type="flex" class="edit-icon">
                        <el-col :span="3"><i @click="addLevel2(item.id)" class="iconfont icon-tianjia"></i></el-col>
                        <el-col :span="3"><i @click="editCategory(item, 1)" style="font-size: 25px"
                                class="iconfont icon-bianji"></i></el-col>
                        <el-col :span="3"><i @click="deleteTopCategory(item.id)" class="iconfont icon-shanchu"></i></el-col>
                    </el-row>
                </el-col>
            </el-row>
            <el-row class="oneBar-content">
                <el-button type="primary" v-for="(secItem, secIndex) in item.children" :key="secIndex"><span
                        @click.prevent="editCategory(secItem, 2)">{{ secItem.name + "(" + secItem.goods.length +
                            ")" }}</span><i @click.prevent="deleteSecCategory(secItem.id)"
                        class="el-icon-close el-icon--right"></i></el-button>
                <div style="display: flex; margin-top: 10px; justify-content: center" v-if="item.children.length === 0">
                    该分类下还没有子栏目
                </div>
            </el-row>
        </el-row>
